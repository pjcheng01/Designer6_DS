...
> 2026-09-11 盤點。本文只做分析與建議，**尚未動任何檔案**。
> 相關背景見 `移植專案交接手冊.md`。
...
| 階段 | 內容 | 風險 | 需要您做的驗證 |
|------|------|------|----------------|
| **1** | 關掉 `campro.ini` 的 `update=1` | 極低 | 開新圖，確認不再有 campro.exe 啟動 |
| **2** | 移除 campro 的 6 個外呼與對應的 5 個選單項 | 低 | 開圖、插圖框、組合圖框資料匯出仍正常 |
| **3** | 刪 `PDMSERVER/` 與 8 支 PDM exe/bat | 極低 | 重開 DraftSight，所有功能如常 |
| **4** | 清 `BOM.lsp` 的死 PDM 分支（8 處） | 中 | **`c:out` 拆圖功能要完整重測** |
| **5** | `bomtree` typ=2 改 .csv，移除 `tree1.exe` | 低 | 跑「結構樹」，確認產出 .csv 且內容正確 |
| **6** | `bomtree` typ=0 改 .csv，移除 `bom1.exe` | 中 | 需先確認次組立名稱回寫還用不用 |
| **7** | 刪 5 支重複／舊版 .lsp | 極低 | 重開 DraftSight |
| **8** | 加密狗相關（`KB/`、`安裝Key程式/`、`SYSDRVR/`） | 極低 | 您決定是否保留授權來源 |
